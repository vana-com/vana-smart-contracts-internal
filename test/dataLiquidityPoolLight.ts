...
import { DAT, DataLiquidityPoolLightImplementation } from "../typechain-types";
...
describe("DataLiquidityPoolLight", () => {
  ...
  describe("Setup", () => {
    ...
    it.only("should have correct params after deploy", async function () {
      (await dlp.name()).should.eq(dlpName);
      (await dlp.owner()).should.eq(owner);
      (await dlp.token()).should.eq(dat);
      (await dlp.fileRegistry()).should.eq(dat);
